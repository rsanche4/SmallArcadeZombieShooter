...
local animationsWalk = {"sprite_zom_walk0_femaleteacher.png", "sprite_zom_walk1_femaleteacher.png", "sprite_zom_walk2_femaleteacher.png", "sprite_zom_walk1_femaleteacher.png"}
local atkAnim = "sprite_zom_atk_femaleteacher.png"
local isAtk = false
...
if (REAPI:readVar("health")>0 and REAPI:abs(event_x-REAPI:getPlayerX())<2 and REAPI:abs(event_y-REAPI:getPlayerY())<2) then
    local hypnosis = {"hypnosis.png", "hypnosisinv.png"}
    REAPI:addUIToScreen(hypnosis[(REAPI:getFrameNumber()%2)+1], 0, 0, 200)
    REAPI:playSE("stunneratk.wav", false)
    isAtk = true
end
